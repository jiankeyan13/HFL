...
from core.utils.registry import UPDATER_REGISTRY
from .base_updater import BaseUpdater


@UPDATER_REGISTRY.register("flame_noise_updater")
class FlameNoiseUpdater(BaseUpdater):
    """
    FLAME 更新器：在更新全局模型后添加高斯噪声。
    噪声标准差 = noise * clip_value（从 context 获取）。
    """
    
    def __init__(self, noise: float = 0.001, config=None):
        # paper：Accordingly, λ = 0.001 for IC and NLP, and λ = 0.01 for the NIDS scenario.(From B.3 Effectiveness of Adding Noise)
        """
        Args:
    ...
    def update(self, 
        ...
        noise_std = self.noise * clip_value
